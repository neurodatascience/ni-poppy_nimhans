...
# This probably can be replaced by checking against manifest "datatype" field
# But good to have addtional check before processing
def filter_missing_datatype_participants(proc_participants, session, datatype, DATASET_ROOT, logger):
    """ Checks if the datatype subdirectory exists within the bids session directory
    """
    proc_participants_with_datatype = []
    for participant_id in proc_participants:
        participant_dataype_dir  = f"{DATASET_ROOT}/bids/{participant_id}/{session}/{datatype}"
        if Path.is_dir(Path(participant_dataype_dir)):
            proc_participants_with_datatype.append(participant_id)
        else:    
            logger.info(f"Skipping participant {participant_id} since {participant_dataype_dir} does not exist")

    return proc_participants_with_datatype
...
for wf in workflows:
    ...
    if wf == "generate_manifest":
        ...
        if n_proc_participants > 0:
            logger.info(f"Running {wf} on {n_proc_participants} participants from session: {session} and for modalities: {modalities}")
        ...
        if n_proc_participants > 0:
            bids_db_path = refresh_bids_db(global_configs, session_id, wf, ignore_patterns, logger)
            logger.info("Checking if participants have func datatype dir")
            proc_participants = filter_missing_datatype_participants(proc_participants, session, "func", DATASET_ROOT, logger)
            # Generate a list of participants to run on HPC 
            if use_hpc:                
                hpc_job_list_file = f"{hpc_job_list_dir}/hpc_job_list_{wf}_{session}.txt"
                logger.info(f"Generating HPC job list for {n_proc_participants} participants: {hpc_job_list_file}")
        ...
        if n_proc_participants > 0:
            logger.info("Checking if participants have dwi datatype dir")
            proc_participants = filter_missing_datatype_participants(proc_participants, session, "dwi", DATASET_ROOT, logger)
            # Generate a list of participants to run on HPC 
            if use_hpc:                
                hpc_job_list_file = f"{hpc_job_list_dir}/hpc_job_list_{wf}_{session}.txt"
                logger.info(f"Generating HPC job list for {n_proc_participants} participants: {hpc_job_list_file}")
